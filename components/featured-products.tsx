    ...
    const loadSweets = async () => {
      try {
        ...
        const fetched = data.map((sweet: any, index: number) => ({
          ...
          image: sweet.image || "https://images.unsplash.com/photo-1590080875515-8a3a8dc5735e?w=400&h=400&fit=crop&q=90",
          rating: 4.6,
          reviews: 50,
                ...
                <div className="relative aspect-[4/3] overflow-hidden">
                  ...
                  <div className="absolute left-3 right-3 top-3 flex items-start justify-between">
                    ...
                    <button onClick={() => toggleFavorite(product.id)}
                      ...
                      <Heart className={`h-5 w-5 transition-colors ${favorites.includes(product.id) ? "fill-[#C0392B] text-[#C0392B]" : "text-[#7A5C44]/60"}`} />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {getQty(product.id) > 0 ? (
                      <div className="flex items-center gap-2 rounded-full bg-gradient-to-r from-[#C0392B] to-[#7B241C] p-1 shadow-md">
                        <button onClick={() => updateQty(product.id, getQty(product.id) - 1)}
                        ...
                        <span className="min-w-[24px] text-center text-sm font-bold text-white">{getQty(product.id)}</span>
